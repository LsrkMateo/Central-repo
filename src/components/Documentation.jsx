  ...
  return (
    <div
      className={`min-h-screen p-8 ${
        dark ? "bg-black text-white" : bgColorClass
      }`}
    >
      <div
        className={`text-4xl font-bold mb-4 ${
          dark ? "text-white" : textColorClass
        }`}
      >
        Documentación:
      </div>
      <div className={`mb-4 ${dark ? "text-gray-400" : textMutedClass}`}>
        ...
        <Link href={"/documentacion/modo-oscuro"} className="boton-link">
          <div
            className={`p-4 rounded shadow-lg ${
              dark ? "bg-gray-800 text-white" : bgColorClass
            } transition-all hover:${
              dark ? "brightness-125" : "brightness-75"
            }`}
          >
